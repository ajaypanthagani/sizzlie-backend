...
var IngredientTableColumns = struct {
	...
	Thumbnail:   "ingredients.thumbnail",
...
var IngredientWhere = struct {
	...
	Name:        whereHelpernull_String{field: "`ingredients`.`name`"},
	Description: whereHelpernull_String{field: "`ingredients`.`description`"},
	Thumbnail:   whereHelpernull_String{field: "`ingredients`.`thumbnail`"},
...
var (
	ingredientAllColumns            = []string{"id", "name", "description", "thumbnail", "created_at"}
	ingredientColumnsWithoutDefault = []string{"name", "description", "thumbnail"}
	ingredientColumnsWithDefault    = []string{"id", "created_at"}
...
func (o *Ingredient) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `ingredients` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `ingredients` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, ingredientPrimaryKeyColumns))
		}
	...
	o.ID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == ingredientMapping["id"] {
		goto CacheNoHooks
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for ingredients")
	}
...
var mySQLIngredientUniqueColumns = []string{
	"id",
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *Ingredient) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	nzDefaults := queries.NonZeroDefaultSet(ingredientColumnsWithDefault, o)
	nzUniques := queries.NonZeroDefaultSet(mySQLIngredientUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "`ingredients`", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `ingredients` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for ingredients")
	}
	...
	uniqueMap, err = queries.BindMapping(ingredientType, ingredientMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for ingredients")
	}
